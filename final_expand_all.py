...
# -*- coding: utf-8 -*-
"""
最终版：所有16个产品 seoDescription 一次性扩充到 350-400 词
"""
...
def main():
    ...
    for prod in data['products']:
        ...
        if tid in all_descs:
            ...
            prod['seoDescription'] = all_descs[tid].strip()
            new_words = count_words(prod['seoDescription'])
            print(f"✅ {tid} {prod['name'][:30]}: {old_words} → {new_words} words")
            updated += 1
    
    print(f"\n已处理 {updated} 个产品")
    
    # 只写了3个，还需要写另外13个
    print(f"\n⚠️  警告：只写了 3 个产品，还有 13 个需要补充")
    print("请继续添加剩余13个产品的描述...")
